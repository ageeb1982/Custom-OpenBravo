...
public class DataLogicCustomers extends BeanFactoryDataSingle {
    ...
    public void init(Session s){
        ...
        tcustomers = new TableDefinition(s
            , "CUSTOMERS"
            //	BR [ 2 ]
            , new String[] { "CUSTOMERS_ID", "TAXID", "SEARCHKEY", "NAME", "NOTES", "ISVISIBLE", "CARD", "MAXDEBT", "CURDATE", "CURDEBT"
                           , "FIRSTNAME", "LASTNAME", "EMAIL", "PHONE", "PHONE2", "FAX"
                           , "ADDRESS", "ADDRESS2", "POSTAL", "CITY_ID", "REGION_ID", "COUNTRY_ID"
                           , "TAXCATEGORY" }
        	//	BR [ 2 ]
            , new String[] { "CUSTOMERS_ID", AppLocal.getIntString("label.taxid"), AppLocal.getIntString("label.searchkey"), AppLocal.getIntString("label.name"), AppLocal.getIntString("label.notes"), "VISIBLE", "CARD", AppLocal.getIntString("label.maxdebt"), AppLocal.getIntString("label.curdate"), AppLocal.getIntString("label.curdebt")
    ...
    public SentenceList getCustomerList() {
        return new StaticSentence(s
                //	BR [ 2 ]
            , new QBFBuilder("SELECT CUSTOMERS_ID, TAXID, SEARCHKEY, NAME FROM CUSTOMERS WHERE ISVISIBLE = " + s.DB.TRUE() + " AND ?(QBF_FILTER) ORDER BY NAME", new String[] {"TAXID", "SEARCHKEY", "NAME"})
    ...
    public int updateCustomerExt(final CustomerInfoExt customer) throws BasicException {
        ...
        return new PreparedSentence(s
                //	BR [ 2 ]
                , "UPDATE CUSTOMERS SET NOTES = ? WHERE CUSTOMERS_ID = ?"
